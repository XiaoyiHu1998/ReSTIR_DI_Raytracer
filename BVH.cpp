...
bool BVH::Traverse(Ray& ray)
{
	...
	for (const BVH_Object& bvhObject : m_BVHObjects)
	{
		...
		ray.nodeIntersectionCount += objectRay.nodeIntersectionCount;
		ray.nodeIntersectionTestCount += objectRay.nodeIntersectionTestCount;
		ray.triangleIntersectionCount += objectRay.triangleIntersectionCount;
		ray.triangleIntersectionTestCount += objectRay.triangleIntersectionTestCount;
		ray.hitCount += objectRay.hitCount;
	}
...
RandomLightPoint BVH::RandomTrianglePoint(uint32_t& seed) const
{
	...
	return RandomLightPoint(trianglePoint.point, randomBVHObject.material.emittedRadiance, randomBVHObject.bvh.Area(), trianglePoint.normal, randomBVHObject.inverseTransform);
}
